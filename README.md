...
- **决策类分流依赖模型判断（跨平台弱点）**：模型会把"怎么修"路由到 MoA，却可能把"这算不算问题"的语义可接受性（如静默接受负数金额 / 重复记录）**自行拍板为"不是问题"**而不立案。`references/issue-rules.md` 已加硬规则要求"静默接受异常输入必须立案、两解语义必进 NEEDS_CONFIRMATION"；该规则的端到端有效性尚未在两平台复测确认（此前两次观察均发生在加规则之前）。
