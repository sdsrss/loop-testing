...
- **D1 基石假设（双平台共用 SKILL.md）验证状态**：本机已核实 codex-cli 0.144.1，且 `~/.codex/skills/` 采用 `<name>/SKILL.md` 目录格式（与本技能布局一致），`install-codex.sh` 落位经沙箱测试通过；**尚未在真实 Codex 会话中实测"安装后技能被加载并触发"**——此为 D1 验证门的剩余开放项。若某版本 Codex 的 skills 机制与此不符，回退方案为向 `~/.codex/prompts` 生成等价 prompt 文件（该目录当前不存在，回退路径待真实环境验证）。
