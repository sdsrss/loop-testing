...
| `NO_PROXY` / `no_proxy` | 逗号分隔的代理豁免列表：`host`（含子域）、`.suffix`、`host:port`、IPv6 字面量（`::1` 或 `[::1]`）、`*`；取两者中第一个非空值。命中的端点（如本地 Ollama/vLLM）直连，`--dry-run` 按端点逐行显示走代理还是直连。**不支持 CIDR 网段**（如 `192.168.0.0/16`，curl 7.86+ 支持）：此类条目不匹配，端点继续走代理（fail-closed），在 `--dry-run` 里可见 |
...
**真实付费调用的授权边界（FR-6.7，非仅"冒烟"）**：只要 env 里存在可用 key（`--dry-run` 显示任一 key `set`），MoA 就会真发**付费 / 外网**委员会调用——这不限于专门的"冒烟测试"，正式循环中**每一次**遇到 NEEDS_CONFIRMATION 而触发的真实调用同样属付费/外网动作，按 FR-6.7 **需先获授权，不在自主推进范围内**。因此派 agent 跑循环前先 `--dry-run` 看 key 状态：显示 `set` 即先获授权，或用 `LOOP_TESTING_MOA_*` 覆盖 / 清空相关 env 让引擎走 exit 2 单模型降级；`--dry-run` 全部 `missing`（引擎必 exit 2）时按上条自主降级，无需授权。**env 里已存在 key 不等于用户已授权本次付费调用**——两者是不同的判断，勿把前者当后者。
