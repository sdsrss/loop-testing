...
3. 本轮开始前已知、到本轮结束仍未 `VERIFIED` 的 P0-P2，均已严格归入 `NEEDS_CONFIRMATION / BLOCKED / WONT_FIX / CANNOT_REPRODUCE` 四者之一（与 `issue-rules.md` §7 状态机逐字一致——少列一个状态，就会让该状态下的问题永远卡住收敛，而 §7 又要求你用它），每条具备完整复现、影响、尝试记录与明确下一步。**`OPEN` / `FIXING` / `FIXED_UNVERIFIED` 都不满足本条**：前两者是还没做完；`FIXED_UNVERIFIED` 是改了但没重放复验，出路是按 `issue-rules.md` §7 先把重放命令与输出写进 `runs/round-N.md` 再标 `VERIFIED`，或如实改判为上面四种之一——不得当作「已解决」。
4. **本轮计划场景均已实际执行**，没有用跳过、阻塞或仅看代码代替。
5. 所有未被已知问题明确阻断的核心/高风险旅程均已覆盖，且本轮**至少完成一条端到端冒烟旅程**。
6. 构建 / 测试 / lint / 类型检查 / 健康检查**无新增失败**；既有失败已与对应问题关联且未恶化。
7. **完成全功能回归，且本轮 `cases_this_round` ≥ 此前所有轮次最大值的 80%**（防「少测凑零新增」）。判读方式，勿凭印象：
   - **数字来源唯一**：读各轮 `runs/round-N.md` 机器字段块里的 `cases_this_round`；`FEATURE_MATRIX.md` 的同名字段是本轮工作副本，轮末必须与本轮 `runs/round-N.md` 一致，冲突时以轮日志为准。
   - **基线是此前所有轮次的最大值**，不是上一轮——只跟上一轮比，连着缩两轮就能把基线一路降下来。
   - 低于 80% 即「覆盖缩水轮」：本轮**不算收敛轮，且 `converged_streak` 归零**（见下行）。
   - **唯一例外**：本轮 `runs/round-N.md` 用一行写明缩掉的是哪些用例、为何不再适用（功能已删除 / 场景已失效 / 此前轮次重复计数），并给出扣除后的可比基数——写清楚了就按未缩水处理，写不清楚就是缩水。

满足 → `converged_streak += 1`；出现 P0-P2 新发现 / 扩大已知重要问题 / 重要回归 / 无理由漏测本轮场景 / 新关键检查失败 / **本轮为覆盖缩水轮（判据 7 未达 80% 且无合格例外说明）** → `converged_streak = 0`。

> 缩水轮**归零**而不是「不计入」：只「不计入」时，A 收敛 → B 缩水 → C 收敛 会得到 `converged_streak = 2` 并就此停止，而 A 与 C 之间隔着一轮没测什么的 B——§2 要求的「连续两轮」从来没有发生过。
...
9. 代码交付：修改文件、**全部会话期间产生的 commit（逐分支列出——若有任何提交落在主分支必须显式披露，不得遗漏）**、未提交变更、最终 `git status`，明确声明**从未 push/merge/release/deploy**。声明某文件"未触碰"前先以 `git diff`/`git log` 核对——工具附带修改（如插件写入的配置）也必须如实列出归因，不得笼统归为"用户自有内容未动"；
