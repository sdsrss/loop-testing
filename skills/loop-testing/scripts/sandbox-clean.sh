    ...
    *)
      ...
      case "$WT_STATE" in
        ...
        legacy)
          ...
          # `-uall` is load-bearing, not tidiness (delta review D6). Without it
          # `--ignored` reports an ignored DIRECTORY as one collapsed entry:
          # `!! config/`, with a `.env` inside it never named. Measured. That is
