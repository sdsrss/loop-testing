...
**提前停掉无人值守长跑:** Ctrl-C、`kill -TERM <driver-pid>`、`SIGQUIT` 或挂断(关终端 / SSH
断线)都会**停掉驱动和它正在跑的会话**,驱动分别以 130 / 143 / 131 / 129 退出。会话跑在自己的
进程组里(`timeout` 会建一个),所以驱动先给那个进程组发信号,**然后等会话真的消失,才释放
`.driver.lock`**。只发信号不够:看门狗会把 `SIGTERM` 转发下去,十五秒后才升级到 `SIGKILL`——
发完信号就退出的驱动,会在一个全权限会话仍在收尾时就把锁放开,第二个驱动正好能在这个缝里
启动。等待上限是 20 秒,即那十五秒的保证再加余量;到点后驱动给会话进程组补发一次 `SIGKILL`。
**等待期间驱动会把会话 pid 和这个上限打到 stderr,再发一次信号会立刻升级到 `SIGKILL`**,
而不是取消等待——所以连按两次 Ctrl-C 的含义是「立刻停」,也绝不会在会话还活着时把锁放开。
若此时会话已消失就释放锁,仍留在会话进程组里的孙进程(agent 留下的 dev server 之类)只记进
`driver.log`,不拖住整个项目;而调用过 `setsid` 的孙进程已经脱离该进程组,既不会被停掉,
也不会被记录。若会话本身连 `SIGKILL` 都没杀掉(病态情况),驱动会**保留**锁并把锁里的 pid
改写成那个进程:留着的锁是看得见的,会拦住下一次运行;放开的锁则会悄悄放行第二个全权限会话
去写同一份 `STATE.md`。对驱动发 `SIGKILL` 会跳过上述全部流程——会话继续跑,锁留在原地且指向
一个已死的 holder,下一个驱动会把它偷走——所以请用上面几种信号。另外:PATH 上没有
`timeout`/`gtimeout` 时驱动**拒绝启动**(否则 wall-clock 看门狗会静默缺失);要在这样的主机上
照样启动、并接受那里没有任何东西给会话设上界,请显式传 `--no-watchdog`。该 flag 豁免的**只有
这条拒绝**:主机上**有**上述二进制时它什么也不授予,每个会话仍被包在低于 `--session-minutes`
的看门狗里,驱动会在 stderr 与 `driver.log` 里明说这一点,而不是默默收下这个 flag。有看门狗
二进制的主机上没有办法让会话无界运行。
