...
cleanup_ws() { [ ${#CLEAN[@]} -gt 0 ] || return 0; chmod -R u+rwX "${CLEAN[@]}" 2>/dev/null; rm -rf "${CLEAN[@]}"; }
...
# F. no temp residue.
WS6=$(mk_proj); TD=$(mktemp -d "${TMPDIR:-/tmp}/loop-testing-cxerrtmp.XXXXXX"); CLEAN+=("$WS6" "$TD")
...
WS7=$(mk_proj); RELHOME=$(mktemp -d "${TMPDIR:-/tmp}/loop-testing-cxrelhome.XXXXXX"); CLEAN+=("$WS7" "$RELHOME")
...
WS8=$(mk_proj); ROTMP=$(mktemp -d "${TMPDIR:-/tmp}/loop-testing-cxrotmp.XXXXXX"); CLEAN+=("$WS8" "$ROTMP")
...
DISP=$(mktemp -d "${TMPDIR:-/tmp}/loop-testing-cxdisp.XXXXXX"); CLEAN+=("$DISP")
